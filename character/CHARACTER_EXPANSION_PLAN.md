...
2026-09-24 角色与配饰增补：新增书签夹、小茶杯、渔夫帽、画家帽，配套两副眼镜、两种轻量脸颊点缀，以及书袋、茶围裙、侧袋、画衣；用阅读、茶点、漫游、绘画 4 个新配方展示。阅读、茶点、漫游分别复用抹茶低身形、摩卡圆身形、草莓微尖身形，绘画复用 Bun。总计 78 个独立 SVG 和 8 个角色配方；所有部件维持原有槽位与 `1200 × 900` 透明画布，不改变行为基因、稀有度与孵化规则。

2026-09-24 美术扩展：Bun / Pudgy 身体复用原有前爪、尾巴和阴影；新增 Sesame / Dapple 花纹与 Sprout Cloud，形成 5 个基础模板和 5 个变异模板。新增 4 件头饰、2 副眼镜、4 件低覆盖率服装，并用园艺、烘焙、观星、手作 4 个配方展示角色搭配。所有新增 SVG 均在现有槽位和 `1200 × 900` 画布上；配方只用于视觉组合，不改变基因、稀有度或孵化规则。具体 ID 以 `modular/manifest.json` 为准。

同日根据视觉参考补充颜色与形态变化：Sesame 改为摩卡色圆润身体，Dapple 改为草莓粉微尖身体，Sprout Cloud 改为抹茶绿低矮身体。每个变异身体分别绘制 body、前爪和尾巴，保留共用的脸部、配饰和运行时图层。三种颜色与三种轮廓是成对设计的变异，不表示已建立任意身体与颜色自由组合的调色板系统。
...
成人 DESK 已使用 `manifest.json` 合成角色，新增穿戴会出现在现有衣橱。新增身体、变异和角色配方目前用于组合预览；它们不会自动进入现有生成与稀有度规则。
